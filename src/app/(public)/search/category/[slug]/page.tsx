...
import { getBlogsByCategorySlug } from '@/lib/api/public/blogs.api';
import { getPublicCategories, getPublicCategory } from '@/lib/fetchers/category.fetcher';
...
export async function generateMetadata({ params }: CategoryPageProps): Promise<Metadata> {
    ...
    try {
        const res = await getPublicCategory(slug);
        const category = res.data;
        if (!category) {
            return buildSeoMetadata({
                title: 'Category Not Found | AllTechTamil',
        ...
        const title = `Articles on ${category.name} | AllTechTamil`;
        const description = category.description || `Read high-quality articles about ${category.name} on the AllTechTamil blog.`;
...
export default async function CategorySearchPage({ params, searchParams }: CategoryPageProps) {
    const { slug } = await params;
    ...
    try {
        ...
        console.error('[CategorySearchPage] Failed to fetch category:', error?.message || error);
    }
    ...
    try {
        const res = await getBlogsByCategorySlug(slug, currentPage, POSTS_PER_PAGE);
        posts = res.data || [];
        totalPages = res.meta?.totalPages || 1;
        totalCount = res.meta?.totalCount || 0;
    } catch (error) {
        console.error('[CategorySearchPage] Failed to fetch category blogs:', error);
    }
    ...
    try {
        ...
        console.error('[CategorySearchPage] Failed to fetch sidebar items:', error);
    }
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-outline-variant/30 pb-8 gap-8">
                    <div className="max-w-2xl">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-xs font-bold text-primary tracking-widest uppercase">Category Archive</span>
                        </div>
                        <h1 className="text-3xl md:text-5xl font-extrabold text-on-surface mb-6">{category.name}</h1>
                        <p className="text-base md:text-lg text-on-surface-variant leading-relaxed">
                            {category.description || `Data-driven insights, framework analysis, and guides listed under the ${category.name} taxonomy.`}
                        </p>
                    </div>
                ...
                <div className="lg:col-span-8 space-y-12">
                    {posts.length === 0 ? (
                        <div className="py-24 text-center bg-surface-container-low rounded-3xl border border-outline-variant/40 px-6 max-w-2xl mx-auto">
                            <Folder className="w-16 h-16 text-primary mx-auto mb-6 opacity-80" />
                            ...
                            <p className="text-on-surface-variant max-w-md mx-auto mb-8 leading-relaxed text-sm">
                                We couldn't find any articles published in this category yet.
                            </p>
                            <Link 
                                href="/" 
                        ...
                        <>
                                        ...
                                        <div className="p-6 flex flex-col flex-grow justify-between">
                                            <div>
                                                <div className="flex items-center gap-3 mb-3 text-xs text-outline font-medium">
                                                    <span className="text-primary font-bold uppercase tracking-wider">{category.name}</span>
                                                    <span>•</span>
                            ...
                            {totalPages > 1 && (
                                <div className="pt-8 flex items-center justify-center gap-2">
                                    {currentPage > 1 ? (
                                        <Link
                                            href={`/search/category/${slug}?page=${currentPage - 1}`}
                                    ...
                                    {Array.from({ length: totalPages }, (_, i) => {
                                        ...
                                        return (
                                            <Link
                                                key={pageNum}
                                                href={`/search/category/${slug}?page=${pageNum}`}
                            ...
                            {categories.slice(0, 8).map((cat) => {
                                ...
                                return (
                                    <Link 
                                        ...
                                        className={`flex items-center justify-between group py-1.5 px-3 rounded-lg transition-all ${
                                            isActive 
                                    ...
                                    >
                                        ...
                                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${
                                            isActive ? 'bg-primary/20 text-primary' : 'bg-surface-container text-outline'
                                        }`}>
                                            {cat.blogsCount ?? 0}
                                        </span>
                                    </Link>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {tags.slice(0, 12).map((t) => (
                                    <Link
                                        ...
                                        className="px-3 py-1 bg-surface-container-low border border-outline-variant/20 rounded-full text-xs font-semibold text-on-surface-variant hover:bg-primary hover:text-on-primary hover:border-primary transition-all duration-200"
                                    >
                                        {t.name}
                                    </Link>
                                ))}
